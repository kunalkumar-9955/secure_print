...
import { CreditCard, Banknote, ShieldCheck, AlertCircle, Loader2, RefreshCw, ArrowLeft, CheckCircle2 } from 'lucide-react';
...
export default function JobPaymentPage() {
  ...
  // Poll job status to detect physical print completion & cash confirmations
  ...
  // Query whether online payment is active for this shop
  const { data: paymentMethods } = useQuery({
    queryKey: ['public-payment-methods', jobId],
    queryFn: () => apiRequest(`/api/v1/payments/public-methods/${jobId}`),
    enabled: !!jobId && jobId !== 'new',
  });
  ...
  const handlePayOnline = async () => {
    ...
    try {
      ...
      // 1. If Cashfree payment session ID is returned, launch official Cashfree JS SDK v3 Dropin
      if (order?.paymentSessionId) {
        let cashfreeFn = (window as any).Cashfree;
        if (typeof cashfreeFn !== 'function') {
          // Wait briefly for Cashfree script to mount if still downloading
          await new Promise<void>((resolve) => {
            let elapsed = 0;
            const check = setInterval(() => {
              elapsed += 100;
              if (typeof (window as any).Cashfree === 'function' || elapsed >= 3000) {
                clearInterval(check);
                resolve();
              }
            }, 100);
          });
          cashfreeFn = (window as any).Cashfree;
        }

        if (typeof cashfreeFn === 'function') {
          const cfMode = order.environment?.toLowerCase() === 'production' ? 'production' : 'sandbox';
          const cashfree = cashfreeFn({ mode: cfMode });
          cashfree.checkout({
      ...
      // 2. If direct payment URL returned (simulation or hosted link)
      if (order?.paymentUrl) {
  ...
  const isOnlineConfigured = paymentMethods?.isOnlineConfigured ?? true;
                ...
                {isOnlineConfigured ? (
                  ...
                  <div className="p-3.5 rounded-xl bg-amber-50 border border-amber-200 text-xs text-amber-800 space-y-1">
                    ...
                    <p className="text-[11px] text-amber-700 pl-5">
                      The shop has not configured Cashfree online payments yet. Please select cash payment below.
                    </p>
                  </div>
                )}
                ...
                >
                  <div className="flex items-center space-x-3">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${
                      !isOnlineConfigured ? 'bg-emerald-100 text-emerald-700' : 'bg-white border border-slate-200 text-slate-600'
                    }`}>
