...
export default function AdminCustomersPage() {
  ...
  const {
    ...
    queryKey: ['admin-shop-customers', shopId],
    queryFn: () => apiRequest(`/api/v1/customers/shop/${shopId}`),
    enabled: !!shopId,
  });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Active and recent customer upload sessions initiated via your shop QR code.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 bg-white border border-slate-200 rounded-2xl p-8 space-y-3">
          ...
          <p className="text-sm text-slate-500 max-w-sm mx-auto">
            When customers scan your counter QR code and enter their name, their sessions will appear here.
          </p>
        </div>
      ) : (
            ...
            <table className="w-full text-left text-sm">
              <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 text-xs uppercase tracking-wider font-semibold">
                <tr>
                  ...
                  <th className="py-3.5 px-4">Session Started</th>
                    ...
                    <tr key={s.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3.5 px-4 text-xs text-slate-500 font-mono">
                        <span className="inline-flex items-center space-x-1">
                          ...
                          <span className="truncate max-w-[200px]">{s.deviceInfo || 'Mobile / Web Browser'}</span>
                        </span>
                      </td>
