...
export default function AdminPrintersPage() {
  ...
  const handleSetDefault = async (printerId: string) => {
    ...
    setFeedback(null);
    try {
      await apiRequest(`/api/v1/printers/${printerId}/default`, {
        method: 'PATCH',
      });
      setFeedback({ msg: 'Default shop printer updated successfully.' });
      await refetch();
    } catch (err: any) {
      setFeedback({ msg: err.message || 'Failed to update default printer.', isError: true });
    } finally {
  ...
  const handleDeletePrinter = async (printerId: string, printerName: string) => {
    if (!confirm(`Are you sure you want to remove printer queue '${printerName}'?`)) return;
    setActionLoading(printerId);
    setFeedback(null);
    try {
      await apiRequest(`/api/v1/printers/${printerId}`, {
        method: 'DELETE',
      });
      setFeedback({ msg: `Printer '${printerName}' removed successfully.` });
      await refetch();
    } catch (err: any) {
      setFeedback({ msg: err.message || 'Failed to remove printer.', isError: true });
    } finally {
      setActionLoading(null);
    }
  };
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl w-full mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">
            Windows Print Spooler queues registered by the SecurePrint native agent
          </p>
        </div>
      ...
      {feedback && (
        ...
        >
          ...
          <button onClick={() => setFeedback(null)} className="text-slate-400 hover:text-slate-600 text-xs font-bold px-2">
            ✕
          </button>
        </div>
      )}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {printers.map((p: any) => {
            ...
            const isWorking = testLoading === p.id || actionLoading === p.id;

            return (
              <div
                key={p.id}
                className={`glass-card p-6 rounded-2xl border shadow-sm space-y-4 bg-white transition-all ${
                  p.isDefault ? 'border-emerald-400 ring-1 ring-emerald-300' : 'border-slate-200'
                }`}
              >
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-slate-900 flex items-center space-x-2">
                        ...
                        {p.isDefault && (
                          <span className="inline-flex items-center space-x-1 text-[10px] bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded-full font-bold">
                  ...
                  <div className="flex items-center space-x-2">
                    {!p.isDefault && (
                      <button
                        ...
                        disabled={isWorking}
                        className="inline-flex items-center space-x-1 px-2.5 py-1.5 rounded-lg border border-slate-300 hover:bg-slate-50 text-slate-700 font-semibold text-xs transition-colors"
                        title="Set as shop-level default printer"
                      >
                    ...
                    <button
                      onClick={() => handleDeletePrinter(p.id, p.windowsPrinterName)}
                      disabled={isWorking}
                      className="p-1.5 rounded-lg text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors"
                      title="Remove printer queue"
                    >
